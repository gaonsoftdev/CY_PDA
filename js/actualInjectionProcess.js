...
var app = new Vue({
	...
	mounted(){
		...
		if(!GX._METHODS_.isLogin()) {
			if(GX.Cookie.get('WkTeamName') != null && GX.Cookie.get('WkTeamName').length > 0){
				...
				//GX.DelayRunManager.add('GoodItemName-focusing', function(){document.querySelector('[name="GoodItemName"]').focus();}, 1);
			}
		}
	},
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			GX.TabIndex.indexing(['GoodItemName','WorkStartTime','WorkEndTime']);
