...
var app = new Vue({
	...
	methods:{
		...
		selectRealNoList: function(){
			var vThis = this;
			vThis.queryForm.RealNoSeq = '';
			let params = {
				'Date': GX.formatDate(this.queryForm.Date, 'YMD'),
				'BizUnit': GX.Cookie.get('BizUnit'),
			};
			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisWPDBottleRealChkAPI_en/RealChkRev')
			.ajax([params], [function(data){
				console.log(data);
				if(data[0] != null){
					vThis.rows.RealNoList = data;
				}
			}], true);
		...
		selectReaSonlSeqList: function(){
			var vThis = this;
			
			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisWPDBottleRealChkAPI_en/RealNameCodeHelp')
			.ajax([{}], [function(data){
				console.log(data);
				if(data[0] != null){
					vThis.rows.ReaSonlSeqList = data;
				}
			}], true);
		...
		add: function(){
			...
			var vThis = this;
			if (vThis.queryForm.OutWHSeq == '' || vThis.queryForm.ReaSonlSeq == '') {
				alert(vThis.langSel.msg.whSel);
				this.codeHelp.BottleNo = '';
				return;
			}
			let params = {
				"BottleNo": this.codeHelp.BottleNo,
			...
			.ajax([params], [function(data){
				...
				if(data[0] != null && data.length == 1){
					...
					let BottleNo = data[0].BottleNo;
					let BottleSeq = data[0].BottleSeq;
					let ItemSeq = data[0].ItemSeq;
					let isDuplicated = false;
					let ItemList = GX.deepCopy(vThis.rows.ItemList);
					let temp = data[0];
					temp.SerialNo = ItemList.length;
					for(let i in ItemList){
						if(ItemList.hasOwnProperty(i) && ItemList[i].BottleNo == BottleNo){
							isDuplicated = true;
							break;
						}
					}

					vThis.codeHelp.BottleNo = '';
					...
					if(isDuplicated) alert(vThis.langSel.msg.duplicate);
						...
						params = {
							'BottleSeq': BottleSeq,
							'Param1': 7,
							'Seq': vThis.queryForm.RealNoSeq,
							'Serl': vThis.queryForm.Rev,
							'Date': GX.formatDate(vThis.queryForm.Date, 'YMD'),
						};
						GX._METHODS_
		...
		remove: function(idx){
			// if(confirm(this.rows.ItemList[idx].SerialNo + '번 항목을 삭제하시겠습니까?')) this.rows.ItemList.splice(idx, 1);
			if(confirm(this.langSel.msg.del)) this.rows.ItemList.splice(idx, 1);
		},
		save: function(){
			var vThis = this;
			let ItemList = GX.deepCopy(this.rows.ItemList);
			if (vThis.queryForm.RealNoSeq == '') {
				alert(vThis.langSel.msg.weekSel);
				return;
			}
			...
			if (vThis.queryForm.ReaSonlSeq == '') {
				alert(vThis.langSel.msg.reasonSel);
				return;
			}
				...
				if(GX.Validation.run('', 'validate')){
					...
					for(let i in ItemList){
						if(ItemList.hasOwnProperty(i) && i > 0){
							param = {};
							param.Rev = vThis.queryForm.Rev;
							param.Date = Date;
							param.ReaSonlSeq = vThis.queryForm.ReaSonlSeq;
							param.BottleSeq = ItemList[i].BottleSeq;
							param.RealNoSeq = vThis.queryForm.RealNoSeq;
							param.WHSeq = vThis.queryForm.OutWHSeq;
					...
					GX._METHODS_
					.setMethodId('Genuine.enModuleName.BisWPDBottleRealChkAPI_en/RealSave')
					.ajax(params, [function(data){
						console.log(data);
		...
		init: function(){
			console.log('init.....');
			this.rows.ItemList = [this.firstData];
			this.codeHelp.BottleNo = '';
		...
		scanBarCode: function(){
			...
			if(obj.value != null && obj.value.length > 0 && event.keyCode == '13'){
				...
				vThis.codeHelp.BottleNo = QRCodeData;
				vThis.add();
		...
		RealNoSeqCh: function () { 
			let target = document.querySelector('[name="RealNoSeq"]');
			this.queryForm.Rev = (target.options[target.selectedIndex].text).replace('차','');
		}
	},
	mounted(){
		const vThis = this;
		GX.Calendar.datePicker('gx-datepicker', {
			...
			callback: function(result, attribute){
				...
				vThis.selectRealNoList();
			}
		});//.set(2022, 1);

		GX.NumberType.init(GX._DATAS_.convertRules);

		this.selectRealNoList();
		this.selectOutWHNameist();
		this.selectReaSonlSeqList();
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('SerialNo').head('{{lang.column[1]}}', 'num')
			.body('<span v-if="index > 0">{{row.SerialNo}}</span>',)
			.item('BottleNo').head('{{lang.column[2]}}', '')
			.body('<span>{{row.BottleNo}}</span><input type="text" v-if="index == 0" id="BottleNo" v-model="codeHelp.BottleNo" gx-scanner="Y"> ', 'code-help')
			.loadTemplate('#grid', 'rows.ItemList');
