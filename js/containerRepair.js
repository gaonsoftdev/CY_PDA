...
var app = new Vue({
	...
	methods:{
					...
					if(data[0] != null && data.length == 1){
						...
						if(isDuplicated) alert(vThis.langSel.msg.duplicate);
							...
							params = {
								'BottleSeq': BottleSeq,
								'Param1': 6,
								'Seq': vThis.queryForm.BottleProcSeq,
								'Serl': 0,
								'Date': '',
							};
							GX._METHODS_
		...
		save: function(){
			...
			if(BottleList.length > 0){
				if(GX.Validation.run('', 'validate')){
					...
					GX._METHODS_
					.setMethodId('Genuine.enModuleName.BisPDRepairBottleAPI/RepairReqSave')
					.ajax(params, [function(data){
						console.log(data);
		...
		bottleProcChange : function(){
			let vThis = this;
			location.href = 'containerRepairComplete.html'
		}
	},
	mounted(){
		const vThis = this;
