...
var app = new Vue({
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('ReqNo',).head('{{lang.column[0]}}', '')
			.item('ReqQty').head('{{lang.column[1]}}', '')
			.item('SerialNo').head('{{lang.column[2]}}', '').body('<button class="btn-style-03" @click="goPacking(index);">{{lang.button[0]}}</button>', 'text-c')
