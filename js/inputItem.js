...
var app = new Vue({
	...
	methods:{
		...
		search: function(){
			...
			let urlPart = ['X','O'];
			console.log(urlPart[params.IsItem]);
			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisMatInPutPDA_en/MatInPutQuery2_1'+urlPart[params.IsItem])
			.ajax([params], [function(data){
				console.log(data);
				if(data[0] != null){
					vThis.rows.InputItemList = data;
					for (i in vThis.rows.InputItemList) {
						if (vThis.rows.InputItemList[i].LotNo != null) {
							vThis.disabledIndex[i] = i;
						} else {
							vThis.disabledIndex[i] = -1;
						}
					}
				}
			}], true);
		...
		goInputMaterialInfo: function (idx) {
			...
			const vThis = this;
			// let params = GX.deepCopy(GX.getParameterValues());
			// Object.assign(params, {ItemSeq:this.rows.InputItemList[idx].ItemSeq});
			// const replaceUrl = location.origin + location.pathname + GX.makeQueryString(params);

			// console.log(replaceUrl);
			// const stateObj = {state: 'selectedItem'};
			// history.replaceState(stateObj, '', replaceUrl);


			GX.LocationHistory.makeUrl('replace', 'goInputMaterialInfo', {ItemSeq:this.rows.InputItemList[idx].ItemSeq});

			let goPageNames = ['inputMaterialInfo','inputProductInfo'];
			let paramData = {};
			paramData.StdUnitQty = vThis.rows.InputItemList[idx].StdUnitQty;
			paramData.ItemSeq = vThis.rows.InputItemList[idx].ItemSeq;
			paramData.WorkOrderSeq = vThis.rows.InputItemList[idx].WorkOrderSeq;
			paramData.WorkOrderSerl = vThis.rows.InputItemList[idx].WorkOrderSerl;
			// location.href = goPageNames[this.params.IsItem] + '.html' + GX.makeQueryString(paramData);
			location.href = goPageNames[vThis.rows.InputItemList[idx].SMAssetGrp == '6008002'?1:0] + '.html' + GX.makeQueryString(paramData);

			// GX.DelayRunManager.add('goInputMaterialInfo', function(){
			// 	let paramData = {};
			// 	paramData.StdUnitQty = vThis.rows.InputItemList[idx].StdUnitQty;
			// 	paramData.ItemSeq = vThis.rows.InputItemList[idx].ItemSeq;
			// 	paramData.WorkOrderSeq = vThis.rows.InputItemList[idx].WorkOrderSeq;
			...
			// 	GX.DelayRunManager.remove('goInputMaterialInfo');
			// }, 500);
		...
		checkDisabled: function(index){
			return Number(this.disabledIndex[index])==Number(index);
		},
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			GX.LocationHistory.setHandler(
				function(eventObj){
					// restoreTask : window.onpageshow : 뒤로가기로 돌아왔을 때 복원 작업
					console.log(eventObj, history.state);
					let ItemSeq = GX.getParameters(true).ItemSeq;
					for(let i in vThis.rows.InputItemList){
						if(vThis.rows.InputItemList.hasOwnProperty(i) && vThis.rows.InputItemList[i].ItemSeq == ItemSeq){
				...
				function(eventObj){
					// beforeGoingTask : window.onpagehide : 페이지를 벗어나서 다른 페이지로 이동하기 전에 작업
				},
				function(eventObj){
					// goBackTask : window.onpopstate : 뒤로가기로 이동하기 전에 작업
				}
			);
			...
			GX.VueGrid
			.init()
			.bodyRow(':class="{\'guide-row\':checkFocus(index) , \'guide-row2\': checkDisabled(index)}"')
			.item('ItemSeq').head('{{lang.column[0]}}', '')
			.item('ItemName').head('{{lang.column[1]}}', 'word-break')
			.item('SerialNo',{rowspan:2}).head('{{lang.column[2]}}', 'text-c')
			.body('<button name="btn" class="btn-style-03" @click="goInputMaterialInfo(index);" :disabled="checkDisabled(index)">{{lang.button[0]}}</button>', 'text-c')
			.nl()
			.item('LotNo').head('{{lang.column[3]}}', '')
			.item('StdUnitQty',{rowsapn:2}).head('{{lang.column[4]}}', 'border-r text-r')
