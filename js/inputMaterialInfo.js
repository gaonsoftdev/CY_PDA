...
var app = new Vue({
	...
	methods:{
		...
		search: function(){
			...
			.ajax([params], [function(data){
				...
				// data = [
				// 	{"LotNo":"90909091","StockQty":10000.00000,"WorkOrderSeq":1003,"WorkOrderSerl":1,"ItemSeq":255},
				// 	{"LotNo":"90909090","StockQty":10000.00000,"WorkOrderSeq":1003,"WorkOrderSerl":1,"ItemSeq":255},
				// 	{"LotNo":"90909094","StockQty":10000.00000,"WorkOrderSeq":1003,"WorkOrderSerl":1,"ItemSeq":255},
				// 	{"LotNo":"90909092","StockQty":10000.00000,"WorkOrderSeq":1003,"WorkOrderSerl":1,"ItemSeq":255}
				// ];
		...
		inputTypeNumber: function(){
			...
			GX.inputTypeNumberEventHandler2(event);
			this.sumQty();
		...
		sumQty: function(){
			...
			for(let i in this.rows.InputMaterialInfoList){
				if(this.rows.InputMaterialInfoList.hasOwnProperty(i) && typeof Number(this.rows.InputMaterialInfoList[i].Qty) === 'number'){
					Qty += Number(this.rows.InputMaterialInfoList[i].Qty);
				}
			}

			this.Qty = GX.NumberType.quantity2(Qty);
		...
		focusLotNo: function(index){
			// document.querySelectorAll('[name="LotNo"]')[index].closest('td').className = '';
			document.querySelectorAll('[name="Qty"]')[index].closest('td').className = '';
			document.querySelectorAll('[name="Qty"]')[index].focus();
			this.focusRowIndex = index;
		},
		blurLotNo: function(index){
			// document.querySelectorAll('[name="LotNo"]')[index].closest('td').className = 'code-help';
		...
		check: function(){
			const vThis = this;
			let params = this.params;
			params.LotNo = this.LotNo;

			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisMatInPutPDA_en/MatInPutBarCodeCheck')
			.ajax([params], [function(data){
				if (data[0] != null) {
					console.log('data',data);
					if(data[0].Status == 0) {
						for(let i in vThis.rows.InputMaterialInfoList){
							if(vThis.rows.InputMaterialInfoList.hasOwnProperty(i)){
								if(vThis.LotNo.indexOf('/'+ vThis.rows.InputMaterialInfoList[i].LotNo + '/') != -1){
		...
		save: function(){
			...
			for(let i in this.rows.InputMaterialInfoList){
				if(this.rows.InputMaterialInfoList.hasOwnProperty(i)){
					if (Number(this.rows.InputMaterialInfoList[i].Qty) > 0) {
						this.rows.InputMaterialInfoList[i].EmpSeq = GX.Cookie.get('EmpSeq');
						this.rows.InputMaterialInfoList[i].UserSeq = GX.Cookie.get('UserSeq');
						this.rows.InputMaterialInfoList[i].DeptSeq = GX.Cookie.get('DeptSeq');
						this.rows.InputMaterialInfoList[i].BizUnit = GX.Cookie.get('BizUnit');
						params.push(this.rows.InputMaterialInfoList[i]);
					}
				}
			}
			...
			if (params.length > 0) {
				...
				GX._METHODS_
				.setMethodId('Genuine.enModuleName.BisMatInPutPDA_en/MatInPutCheck2_1X')
				.ajax(params, [function(data){
					console.log(data);
					if(data[0] != null){
						
						if (data[0].Status == 0 || confirm(vThis.lang.msg[0])) { 
							GX._METHODS_
		...
		scanBarCode: function(){
			...
			if (!event.target.hasAttribute('gx-input')) { 
				
				let obj = document.querySelectorAll('[gx-scanner="Y"]')[0];
				obj.focus();
				// let clipboardData = event.clipboardData || window.clipboardData || document.querySelectorAll('[gx-scanner="Y"]')[0].value;
				if(obj.value != null && obj.value.length >0 && keyCode == '13'){
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('LotNo').head('{{lang.column[0]}}', '')
			//.body('{{row.LotNo}}', 'code-help')
			
			.item('InOutDate').head('{{lang.column[1]}}', 'text-c')
			.item('StockQty').head('{{lang.column[2]}}', 'text-r')
			.item('Qty').head('{{lang.column[3]}}', 'text-r')
			.body('<input type="text" name="Qty" v-model="row.Qty" @blur="blurLotNo(index);" @keydown="inputTypeNumber();" @keyup="inputTypeNumber();" gx-input="Y">', 'code-help')
