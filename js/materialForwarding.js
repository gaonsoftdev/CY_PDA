...
var app = new Vue({
	...
	methods:{
		...
		selectOutReqNo: function(){
			var vThis = this;
			let params = {
				"OutReqNo": this.queryForm.OutReqNo,
			};
			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisWSIAPITEST_HGH_en/MMOutReqNoQuery')
				.ajax([params], [function (data) {
					console.log(data);
					if (data[0] != null) {
						if (data[0].Status == 99999) {
							vThis.queryForm.OutReqNo = '';
							alert(vThis.langSel.msg.numSel);
						} else {
							vThis.queryForm.OutReqNo = data[0].OutReqNo;
							vThis.queryForm.OutReqSeq = data[0].OutReqSeq;
							document.querySelectorAll('[gx-scanner="Y"]')[1].focus();
						}
					}
				}], true);
		...
		save: function(){
			...
			// if (vThis.queryForm.OutWHSeq == '' || vThis.queryForm.InWHSeq == '') {
			// 	alert('창고를 선택하세요.');
			// 	return;
			// }
				...
				if(GX.Validation.run('', 'validate')){
					...
					for(let i in ItemList){
						if(ItemList.hasOwnProperty(i) && i > 0){
							param = {};
							param.OutReqSeq = ItemList[i].OutReqSeq;
							param.OutDate = Date;
							param.OutReqItemSerl = ItemList[i].OutReqItemSerl;
							param.OutWHSeq = ItemList[i].OutWHSeq;
							param.MatItemSeq = ItemList[i].MatItemSeq;
							param.LotNo = ItemList[i].LotNo;
							param.StockQty = Number(ItemList[i].StockQty);
					...
					GX._METHODS_
					.setMethodId('Genuine.enModuleName.BisWSIAPITEST_HGH_en/MMOutProc')
					.ajax(params, [function(data){
						console.log(data);
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			// .item('MatItemNo').head('자재번호', '') //자재번호
			// .body('<span>{{row.MatItemNo}}</span><input type="text" v-if="index == 0" id="MatItemNo" v-model="codeHelp.MatItemNo" gx-scanner="Y"> ', 'code-help')
			// .item('MatItemName').head('자재명', '') //자재명
			// .item('StockQty',{rowspan:2}).head('수량', '')
			// .body('<span>{{row.StockQty}}</span><input type="text" v-if="index != 0" name="StockQty" v-model="rows.ItemList[index].StockQty" gx-scanner="Y"> ', 'code-help')
			// .nl()
			// .item('LotNo',{colspan:2}).head('LotNo', 'border-r').body('{{row.LotNo}}', 'text-c border-r')
