...
var app = new Vue({
	...
	methods:{
		...
		add: function(){
			...
			let params = {
				"ItemNo": this.codeHelp.ItemNo,
				"InWHSeq": this.queryForm.InWHSeq,
				"OutWHSeq": this.queryForm.OutWHSeq,
			};
		
			GX._METHODS_
			.setMethodId('Genuine.enModuleName.BisPDItemMoveAPI/ItemCodeHelp')
			.ajax([params], [function(data){
				console.log(data);
				if(data[0] != null && data.length == 1){
					
					let ItemNo = data[0].ItemNo;
					let LotNo = data[0].LotNo;
					let OutWHSeq = data[0].OutWHSeq;
					let isDuplicated = false;
					let ItemList = GX.deepCopy(vThis.rows.ItemList);
					let temp = data[0];
					temp.SerialNo = ItemList.length;
					for(let i in ItemList){
						if(ItemList.hasOwnProperty(i) && ItemList[i].ItemNo == ItemNo){
							isDuplicated = true;
							break;
						}
					}
					...
					if(isDuplicated) alert(vThis.langSel.msg.duplicate);
						...
						GX._METHODS_
						.setMethodId('Genuine.enModuleName.BisPDItemMoveAPI/ErrorCheck')
						.ajax([params], [function(data){
							console.log(data);
		...
		save: function(){
			...
			if(ItemList.length > 1){
				if(GX.Validation.run('', 'validate')){
					...
					GX._METHODS_
					.setMethodId('Genuine.enModuleName.BisPDItemMoveAPI/SaveAPI')
					.ajax(params, [function(data){
						console.log(data);
		...
		scanBarCode: function(){
			...
			if(obj.value != null && obj.value.length > 0 && event.keyCode == '13'){
				if(vThis.queryForm.OutWHSeq == ''){
					...
					return false;
				}else if(vThis.queryForm.InWHSeq == ''){
					alert('입고창고가 선택되지 않았습니다.');
					return false;
				}
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('ItemNo').head('{{lang.column[2]}}', '') //자재번호
			.body('<span>{{row.ItemNo}}</span><input type="text" v-if="index == 0" id="ItemNo" v-model="codeHelp.ItemNo" gx-scanner="Y"> ', 'code-help')
			.item('ItemName').head('{{lang.column[3]}}', '') //자재명
			...
			.item('StockQty',{rowspan:2}).head('{{lang.column[4]}}', '')
			.nl()
			.item('LotNo',{colspan:2}).head('{{lang.column[5]}}', 'border-r').body('{{row.LotNo}}', 'text-c border-r')
