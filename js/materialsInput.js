	...
	methods:{
		...
		search: function(){
			...
			if(String(this.queryForm.Date).length > 0 && String(this.queryForm.WorkCenterSeq).length > 0){
				...
				GX.LocationHistory.makeUrl('replace', 'search', this.queryForm);
				...
				GX._METHODS_
				.setMethodId('Genuine.enModuleName.BisMatInPutPDA_en/MatInPutQuery1')
				.ajax([params], [function(data){
					console.log(data);
					if(data[0] != null){
						vThis.rows.ItemList = data;
					}
					else vThis.rows.ItemList = [];
				}], true);
			}
		...
		//packing화면 이동
		goProcess: function (idx) {
			//WorkCenterSeq
			let params = GX.deepCopy(this.queryForm);
			params.ItemSeq = this.rows.ItemList[idx].ItemSeq;

			GX.LocationHistory.makeUrl('replace', 'goProcess', params);

			location.href = 'processInformation.html?LotNo=' + this.rows.ItemList[idx].LotNo;
		},
		selectAll: function(){
		...
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			GX.LocationHistory.setHandler(
				function(eventObj){
					...
					for(let i in vThis.rows.ItemList){
						if(vThis.rows.ItemList.hasOwnProperty(i) && vThis.rows.ItemList[i].ItemSeq == ItemSeq){
							vThis.focusRowIndex = i;
							break;
						}
					}

					GX.DelayRunManager.add('search', function(){
						vThis.search();
						GX.DelayRunManager.remove('search');
					}, 500);
				},
				function(eventObj){
					console.log('beforeGoingTask');
			...
			let sDate = (this.params.Date == null) ? GX.nowDate().full : this.params.Date;
			this.queryForm.Date = GX.formatDate(sDate, 'Y-M-D');
			if(this.params.WorkCenterSeq != null) this.queryForm.WorkCenterSeq = this.params.WorkCenterSeq;
			...
			.item('ItemName',).head('{{lang.column[0]}}', '')
			.item('LotNo',{rowspan:2}).head('{{lang.column[1]}}', '')
			.item('SerialNo',{rowspan:2}).head('{{lang.column[2]}}', '').body('<button class="btn-style-03" @click="goProcess(index);">선택</button>', 'text-c')
			.nl()
			.item('OrderQty','').head('{{lang.column[3]}}', 'border-r').body('{{row.OrderQty}}', 'text-r border-r')
