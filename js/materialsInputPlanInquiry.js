...
var app = new Vue({
	...
	methods:{
		...
		selectedEdit: function(index){
			//console.log('index='+index);
			var vThis = this;
			GX.doubleClickRun(event.target, function(){
				let queryData = GX.deepCopy(vThis.rows.ListQuery);
				let params = {};
				params.QryType = 'JumpInQuery';//JumpQuery ???
				params.IDX_NO = queryData[index].SerialNo;//'1';
				params.WorkReportSeq = queryData[index].WorkReportSeq;//'0';
				location.href = GX.makeBaseUrl('/materialsInputPlan.html') + GX.makeQueryString(params);
			});
		},
		init: function(){
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			GX.VueGrid
			.bodyRow('@click="selectedEdit(index); checkedMark(index);" :class="{\'check\':isChecked(index)}"')
