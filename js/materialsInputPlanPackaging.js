...
var app = new Vue({
	...
	methods:{
		...
		setJumpInQuery: function(params){
			...
			.ajax([params], [function(data){
				if(data != null && data.length > 0) {
					...
					data = GX.reductionItemByRows(Object.keys(vThis.addForm), GX.deepCopy(data));
					if(data[0].Qty != null) data[0].Qty = GX.NumberType.quantity(data[0].Qty);
					if(data[0].StdUnitQty != null) data[0].StdUnitQty = GX.NumberType.quantity(data[0].StdUnitQty);
					if(data[0].WorkOrderNo != null) data[0].WorkOrderNo = '';
					if(data[0].WorkOrderSeq != null) data[0].WorkOrderSeq = '0';
					if(data[0].WorkOrderSerl != null) data[0].WorkOrderSerl = '0';
					if(data[0].WorkReportSeq != null) data[0].WorkReportSeq = '0';

					vThis.addForm = data[0];
				}
			...
			if(inputData.Qty.length > 0){
				...
				.ajax([params], [function(data){
					...
					if(data != null && data.length > 0) {
						...
						if(data[0].Qty != null) vThis.addForm.Qty = GX.NumberType.quantity(data[0].Qty);
		...
		add: function(){
			if(GX.Validation.run('', 'validate')){
				let nowTime = GX.formatDate(GX.nowDate().full, 'HI');
						...
						if(isExist){
							for(var di in data){
								if(data.hasOwnProperty(di)){
									...
									if(data[di].Qty != null) data[di].Qty = GX.NumberType.quantity(data[di].Qty);
								}
							}
	...
	mounted(){
		...
		if(GX.Cookie.get('WkTeamName') != null && GX.Cookie.get('WkTeamName').length > 0){
			...
			this.addForm.WorkCenterName = GX.Cookie.get('PackageWorkCenterName');
			this.addForm.WorkCenterSeq = GX.Cookie.get('PackageWorkCenterSeq');
