...
var app = new Vue({
	...
	methods:{
			...
			.ajax([params],[
				function(data){
					if(data != null){
						...
						vThis.QtyData.OutQty = data[0].OutQty;
						vThis.QtyData.InQty = data[0].Qty;
		...
		BarcodeSel: function(){
			var vThis = this;
			if(vThis.queryForm.MoveNo != '' && vThis.queryForm.BarCode != '' && Number(vThis.QtyData.InQty) > Number(vThis.QtyData.MovQty)){
				let params = {QryType:'BarCodeQuery',Gubun:'IDD_MOVE_IN',InOutSerl:'3'};
				...
				.ajax([params],[
					function(data){
						if(data != null){
							if(vThis.queryForm.MoveNo != null) {
								data[0].MoveNo = vThis.queryForm.MoveNo;
								data[0].SerialNo =  vThis.queryForm.MoveNo.split('-')[0];
							}
							if(vThis.queryForm.BarCode != null) data[0].BarCode = vThis.queryForm.BarCode;
							data[0].Gubun = 'IDD_MOVE';
							if(data[0].Qty != null){
								data[0].MovQty = data[0].Qty;
								vThis.QtyData.MovQty = Number(data[0].Qty) + Number(vThis.QtyData.MovQty);
							} 
							data[0].OutQty = vThis.rows.MoveData[0].Qty;
							data[0].InQty = vThis.rows.MoveData[0].InQty;
							data[0].OutWhSeq = vThis.rows.MoveData[0].OutWhSeq;	
		...
		save2: function(){
			var vThis = this;
			let params = {QryType:'NewSave',SendType:''};
			let newData = [];
			this.rows.Query[0].Gubun = 'IDD_MOVE_IN';
			...
			if(clipboardData != null){
				...
				if(isFocused) {
					...
					if(scannerObj.length > 0){
						if(vThis.queryForm[scannerObj[0].name] == '' || vThis.queryForm[scannerObj[0].name] == null){
							activeObjName = scannerObj[0].name;
							activeTabIdx = scannerObj[0].tabIndex;
							document.querySelector('[name="BarCode"]').focus();
							isChange = 'MoveNo';
						} else if(scannerObj.length > 1 && vThis.queryForm[scannerObj[0].name] != ''){
				...
				if(activeObjName.length > 0){
					...
					if(isChange =='MoveNo'){
						document.querySelector('[name="MoveNo"]').value = QRCodeData;
						vThis.openMoveList();
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('MoveNo').head('이동번호', '')
			.item('Gubun').head('구분', '')
			.item('UnitName').head('품명', '')
			.item('ItemNo').head('품번', '')
			.item('LotNo').head('바코드', '')
			//.item('').head('단위', '')
			.item('Qty').head('수량', '')
			.item('ProdDate').head('일자', '')
