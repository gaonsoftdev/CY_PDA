...
var app = new Vue({
	...
	methods:{
		...
		BarcodeSel: function(){
			...
			bk1: if(vThis.queryForm.BarCode != ''){
				...
				for(let i in vThis.rows.Query){
					if(vThis.rows.Query[i].PalletNo == params.BarCode){
						alert('이미 스캔된 바코드입니다.');
						break bk1;
					}
				}
				...
				.ajax([params],[
					function(data){
						if(data != null){
							...
							vThis.queryForm.OutWhSeq = data[0].OutWhSeq;
							vThis.queryForm.OutWhName = data[0].OutWhName;
							vThis.queryForm.UnitSeq = data[0].UnitSeq;
		...
		init: function(){
			...
			this.rows.Query = [];
			this.rows.itemNoListQuery = [];
			this.rows.itemNameListQuery = [];
			this.rows.QuoutWhNameListQueryery = [];
			...
			if(clipboardData != null){
				...
				if(isFocused) {
					...
					if(scannerObj.length > 0){
						if(QRCodeData.includes('P') || QRCodeData.includes('R') || QRCodeData.includes('H')){
							this.init();
		...
		searchCodeHelp: function(targetName, isOnePick){
			...
			params.QryType = targetName+'CodeHelp';
			if(targetName == 'OutWhName') params.QryType = 'WhCodeHelp';
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.loadTemplate('#grid-outwhname', 'rows.outWhNameListQuery');
