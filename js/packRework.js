...
var app = new Vue({
	...
	methods:{
		...
		ReWorkPalletSel: function(){
			var vThis = this;
			let params = {QryType:'RePalletQuery',Gubun:'IDD_PACK'};
		...
		InputPalletSel: function(){
			...
			bk1: if(vThis.queryForm.ReWorkPallet != '' && vThis.queryForm.InputPallet != ''){
				let params = {QryType:'InputPalletQuery',Gubun:'IDD_PACK'};
				let inputData = GX.deepCopy(GX.makeParamByForm(['ReWorkPallet','InputPallet'])); //참조없는 복사
				Object.assign(params, inputData);
				for(let i in vThis.rows.Query){
					if(vThis.rows.Query[i].PalletKey.replaceAll(' ','') == params.InputPallet.replaceAll(' ','')){
						alert('이미 스캔된 바코드입니다.');
						break bk1;
					}
				}
				...
				.ajax([params],[
					function(data){
						if(data != null){
							...
							for(i in data){
								data[i].Date = vThis.queryForm.Date;
		...
		save2: function(){
			var vThis = this;
			let params = {QryType:'NewSave',Gubun:'IDD_PACK',SendType:'3'};
			let newData = [];
