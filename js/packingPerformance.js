...
var app = new Vue({
	...
	methods:{
		...
		search: function(){
			...
			if(String(this.queryForm.Date).length > 0 && String(this.queryForm.WorkCenterSeq).length > 0){
				...
				GX._METHODS_
				.setMethodId('Genuine.enModuleName.BisPDPackagReportPDA_en/WorkOrderQuery')
		...
		scanBarCode: function(){
			...
			if(clipboardData != null){
				...
				vThis.queryForm.BottleNo = QRCodeData;
				//vThis.bottleSel();
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('SerialNo',{rowspan:2}).head('{{lang.column[2]}}', '').body('<button class="btn-style-03" @click="goPacking(index);">{{lang.button[0]}}</button>', 'text-c')
			.nl()
