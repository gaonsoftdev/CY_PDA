...
var app = new Vue({
	...
	methods:{
		...
		DateSel: function(){
			var vThis = this;
			bk1: if(vThis.queryForm.BarCode != ''){
				let params = {QryType:'DateQuery',Gubun:'IDD_REQ_HIS'};
				let inputData = GX.deepCopy(GX.makeParamByForm(['FromDate','ToDate'])); //참조없는 복사
				Object.assign(params, inputData);
				params.FromDate = (params.FromDate.length > 0) ? GX.formatDate(params.FromDate, 'YMD') : '';
				params.ToDate = (params.ToDate.length > 0) ? GX.formatDate(params.ToDate, 'YMD') : '';
				GX._METHODS_
				.setMethodId('ReqHis')
				.ajax([params],[
					function(data){
						if(data != null){
							for(let i in data){
								data[i].OutQty = GX.NumberType.quantity(data[i].OutQty); 
								data[i].ScanQty = GX.NumberType.quantity(data[i].ScanQty);
							}
							vThis.rows.Query2 = data;
							vThis.initSelected();
		...
		InOutNoSel: function(){
			var vThis = this;
			bk1: if(vThis.queryForm.BarCode != ''){
				let params = {QryType:'OutNoQuery',Gubun:'IDD_REQ_HIS'};
				let inputData = GX.deepCopy(GX.makeParamByForm(['InOutNo'])); //참조없는 복사
				Object.assign(params, inputData);
				GX._METHODS_
				.setMethodId('ReqHis')
				.ajax([params],[
					function(data){
						if(data != null){
							for(let i in data){
								data[i].OutDate = vThis.YMDFormatter(data[i].OutDate);
							}
		...
		InvNoSel: function(){
			var vThis = this;
			bk1: if(vThis.queryForm.BarCode != ''){
				let params = {QryType:'InvNoQuery',Gubun:'IDD_REQ_HIS'};
				let inputData = GX.deepCopy(GX.makeParamByForm(['InvNo'])); //참조없는 복사
				Object.assign(params, inputData);
				GX._METHODS_
				.setMethodId('ReqHis')
				.ajax([params],[
	   ...
	   ClickSel: function(index){
			var vThis = this;
			if(event.target.closest('tr') != null) {
				if(vThis.rows.Query2[index].InOutType == '이동'){
					document.querySelector('[name="InOutNo"]').value = 'MO:' + vThis.rows.Query2[index].NoSerl;
					vThis.InOutNoSel();
				}else{
					document.querySelector('[name="InvNo"]').value = 'IV:' + vThis.rows.Query2[index].NoSerl;
					vThis.InvNoSel();
				}
			}
	   },
		...
		init: function(){
			...
			this.queryForm.FromDate = GX.formatDate(GX.nowDate().full, 'Y-M-D');
			this.queryForm.ToDate = GX.formatDate(GX.nowDate().full, 'Y-M-D');
			GX.initForm('addForm');	
			...
			if(clipboardData != null){
				...
				if(scannerObj.length > 0){
					...
					if(QRCodeData.includes('MO')){
						...
						document.querySelector('[name="InOutNo"]').focus();
						isChange = 'InOutNo';
					} else {
						activeObjName = scannerObj[1].name;
						activeTabIdx = scannerObj[1].tabIndex;
						document.querySelector('[name="InvNo"]').focus();
						isChange = 'InvNo';
					}
				}
				...
				if(activeObjName.length > 0){
					...
					if(isChange =='InOutNo'){
						document.querySelector('[name="InOutNo"]').value = QRCodeData;
						vThis.InOutNoSel();
					}else if(isChange =='InvNo'){
						document.querySelector('[name="InvNo"]').value = QRCodeData;
						vThis.InvNoSel();
					}
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			GX.VueGrid
			.init()
			.bodyRow('@click="ClickSel(index);"')
			//.bodyRow('@click="selectGrid(index); search2(index);"')
			//.item('SerialNo').head('번호', 'num')
			.item('OutDate').head('출고일', '')
			.item('NoSerl').head('출고증/명세서번호', '')
			.item('InOutType').head('구분', '')
			.item('WhName').head('출고창고', '')
			.item('OutQty').head('출고수량', '')
			.item('ScanQty').head('스캔수량', '')
			.loadTemplate('#grid2', 'rows.Query2');
...
function responseAppInOutNo(action, result){
	if(action == 'QRCodeScanner'){ // scanner 관련
		if(typeof result == 'object' && result.param == 'once') {
			app.queryForm.InOutNo = result.result;
			// GX.TabIndex.next('ItemNo');
		}
	}
}

function responseAppInvNo(action, result){
	if(action == 'QRCodeScanner'){ // scanner 관련
		if(typeof result == 'object' && result.param == 'once') {
			app.queryForm.InvNo = result.result;
		}
	}
}
