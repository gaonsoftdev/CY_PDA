...
var app = new Vue({
	...
	methods:{
		...
		BarCodeSel: function(){
			...
			bk1: if(vThis.queryForm.BarCode != ''){
				let params = {QryType:'BarCodeQuery',Gubun:'IDD_PAL_HIS'};
				...
				.ajax([params],[
					function(data){
						if(data != null){
							for(let i in data){
								data[i].DateFormat = vThis.YMDFormatter(data[i].ToDate) + ' ' + data[i].ToTime;
							}
	...
	created(){
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			.item('').head('재출고여부', '')
			.item('WhName').head('창고', '')
			.item('CustName').head('거래처', '')
			.item('DateFormat').head('일시', '')
