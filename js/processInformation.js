...
var app = new Vue({
	...
	methods:{
		...
		checkProductInput: function(){
			this.isProductInputs = event.target.checked ? [1] : [0];
		},
		...
		goInputItem: function (idx) {
			...
			let paramMap = {};
			paramMap.IsItem = (this.isProductInputs.indexOf(1) != -1) ? '1' : '0';
			paramMap.ProcSeq = this.rows.ProcessList[idx].ProcSeq;
			GX.LocationHistory.makeUrl('replace', 'goInputItem', paramMap);
		...
		checkDisabled: function(index){
			return Number(this.rows.ProcessList[index].IsPut) == 1;
		},
		...
		if(!GX._METHODS_.isLogin()) location.replace('login.html');
			...
			GX.LocationHistory.setHandler(
				function(eventObj){
					...
					let ProcSeq = GX.getParameters(true).ProcSeq;
					for(let i in vThis.rows.ProcessList){
						if(vThis.rows.ProcessList.hasOwnProperty(i) && vThis.rows.ProcessList[i].ProcSeq == ProcSeq){
			...
			if(this.params.IsItem != null) this.isProductInputs = [this.params.IsItem];
			...
			.item('ProcSeq').head('{{lang.column[0]}}', '')
			.item('ProcName').head('{{lang.column[1]}}', '')
			.item('SerialNo').head('{{lang.column[2]}}', '')
			.body('<button class="btn-style-03" @click="goInputItem(index);" :disabled="checkDisabled(index)">{{lang.button[0]}}</button>', 'text-c')
